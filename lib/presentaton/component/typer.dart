...
class TypeText extends StatefulWidget {
  const TypeText(this.text,
      ...
      this.delayedDuration = Duration.zero});
...
class TypeTextState extends State<TypeText> {
  ...
  delayed() => Future.delayed(widget.delayedDuration, () => restartTimer());
  void restartTimer() {
    timer?.cancel();
    timer = Timer.periodic(
      Duration(
        microseconds: widget.duration.inMicroseconds ~/ widget.text.codeUnits.length + 1,
      ),
      (timer) {
        final typeTextLength = typedText.codeUnits.length;
        final widgetTextLength = widget.text.codeUnits.length;
        if (typeTextLength == widgetTextLength || currentIdx >= widgetTextLength) {
          timer.cancel();
